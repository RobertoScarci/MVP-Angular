    ...
    <app-wizard>
      ...
      <div class="card" [@cardAnimation]>
        <div class="step-header">
          ...
          <h2>Qual è la tua soluzione?</h2>
        </div>
        <p class="step-description">
          Descrivi come la tua startup risolve il problema. Spiega la soluzione in modo chiaro, come funziona, e perché è migliore delle alternative esistenti.
        </p>
        
        <form [formGroup]="form" (ngSubmit)="onSubmit()">
          <mat-form-field appearance="outline" class="full-width" [class.focused]="isFieldFocused">
            <mat-label>Soluzione</mat-label>
            <textarea 
              matInput 
              formControlName="solution" 
              rows="6" 
              placeholder="Descrivi la tua soluzione..."
            ...
            <mat-error *ngIf="form.get('solution')?.hasError('required')">
              La soluzione è obbligatoria
            </mat-error>
            <mat-error *ngIf="form.get('solution')?.hasError('minlength')">
              Minimo 50 caratteri
            </mat-error>
            <mat-error *ngIf="form.get('solution')?.hasError('maxlength')">
...
export class StepSolutionComponent implements OnInit, OnDestroy {
  form!: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.form.get('solution')?.valueChanges.pipe(
      debounceTime(500),
      distinctUntilChanged(),
      takeUntil(this.destroy$)
    ).subscribe(value => {
      if (this.form.valid) {
        this.stateService.updateFormData({ solution: value });
      }
    });
  }
  ...
  get characterCount(): number {
    return this.form.get('solution')?.value?.length || 0;
  }

  onSubmit(): void {
    if (this.form.valid) {
      this.stateService.updateFormData({ solution: this.form.value.solution });
